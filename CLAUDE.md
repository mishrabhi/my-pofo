# CLAUDE.md — Abhishek's Portfolio
...
**Abhishek's Port** is a personal portfolio website built as a single-page application (SPA). It features a neo-brutalist aesthetic with advanced interactive elements: a matrix rain hero animation, an interactive terminal interface, GitHub contribution graph, and WhatsApp-integrated contact form.
...
`GithubGraph.tsx` uses `react-github-calendar` with the username `mishrabhi`. Update the username prop directly in that file if the GitHub account changes.
